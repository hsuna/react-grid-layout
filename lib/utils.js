 ...
 */
export function collides(l1: LayoutItem, l2: LayoutItem, margin: Array): boolean {
  if (l1.i === l2.i) return false; // same element
  if (l1.x + l1.w + margin[0] <= l2.x) return false; // l1 is left of l2
  if (l1.x >= l2.x + l2.w + margin[0]) return false; // l1 is right of l2
  if (l1.y + l1.h + margin[1] <= l2.y) return false; // l1 is above l2
  if (l1.y >= l2.y + l2.h + margin[1]) return false; // l1 is below l2
...
export function compact(
  ...
  for (let i = 0, len = sorted.length; i < len; i++) {
    ...
    if (!l.static && !l.fixed) {
      l = compactItem(compareWith, l, compactType, cols, filterFixed(sorted), margin);
...
function resolveCompactionCollision(
  ...
  for (let i = itemIndex + 1; i < layout.length; i++) {
    ...
    // Ignore static items
    if (otherItem.static || otherItem.fixed) continue;

    // Optimization: we can break early if we know we're past this el
    // We can do this b/c it's a sorted layout
    if (otherItem.y > item.y + item.h) break;

    if (collides(item, otherItem, margin)) {
...
export function compactItem(
  ...
  if (compactV) {
    ...
    while (l.y > 0 && !getFirstCollision(compareWith, l, margin)) {
      l.y--;
    }
  } else if (compactH) {
    // Move the element left as far as it can go without colliding.
    while (l.x > 0 && !getFirstCollision(compareWith, l, margin)) {
      l.x--;
    }
  }
  ...
  let collides;
  while ((collides = getFirstCollision(compareWith, l, margin))) {
    if (compactH) {
      resolveCompactionCollision(fullLayout, l, collides.x + collides.w + margin[0], "x", margin);
    } else {
      resolveCompactionCollision(fullLayout, l, collides.y + collides.h + margin[1], "y", margin);
    }
    // Since we can't grow without bounds horizontally, if we've overflown, let's move it down and try again.
    if (compactH && l.x + l.w + margin[0] > cols) {
      l.x = cols - l.w - margin[0];
      l.y++;
    }
  }
...
export function correctBounds(
  ...
  for (let i = 0, len = layout.length; i < len; i++) {
    ...
    if (!l.static && !l.fixed) collidesWith.push(l);
    else {
      // If this is static and collides with other statics, we must move it down.
      // We have to do something nicer than just letting them overlap.
      while (getFirstCollision(collidesWith, l, margin)) {
        l.y++;
      }
    }
  }
...
export function getFirstCollision(
  ...
  for (let i = 0, len = layout.length; i < len; i++) {
    if (collides(layout[i], layoutItem, margin)) return layout[i];
  }
}
...
export function getAllCollisions(
  ...
  return layout.filter(l => collides(l, layoutItem, margin));
}
 ...
 * @return {Array}        Array of static layout items..
 */
export function filterFixed(layout: Layout): Array<LayoutItem> {
...
export function moveElement(
  ...
  const collisions = getAllCollisions(sorted, l, margin);
...
export function moveElementAwayFromCollision(
  ...
  if (isUserAction) {
    ...
    if (!getFirstCollision(layout, fakeItem, margin)) {
      log(
...
export function synchronizeLayoutWithChildren(
  ...
  // Correct the layout.
  const correctedLayout = correctBounds(layout, { cols: cols }, margin);
  return allowOverlap
    ? correctedLayout
    : compact(correctedLayout, compactType, cols, margin);
}
